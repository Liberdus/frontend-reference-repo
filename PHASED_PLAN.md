...
Tasks:
- [x] Create `ParametersTab` class in `js/components/parameters-tab.js`
- [x] Add a new tab button and panel (`Parameters`) in `index.html`
- [x] Instantiate: `const parametersTab = new ParametersTab();` and load in `app.js`
...
Tasks:
- [x] Add a `refresh()` method to fetch and render details
- [x] Format addresses (short + copy)
- [x] Format amounts (LIB with 18 decimals) and durations (human readable)
...
Tasks:
- [x] Add styles to `base.css` (or a new `parameters.css`)
- [x] Add loading and error states (toast + inline placeholders)
- [x] Respect tx-enabled gating (read-only OK without wallet)
...
**Goal:** Fix critical bugs and improve data accuracy.

### Step 10.1 - Mint Value Display Fix

**Issue:** Mint proposals displayed incorrect values (e.g., "0.000000000003 LIB" instead of "3000000 LIB").

**Root Cause:** 
- The contract's `_executeMint()` function ignores the `value` parameter and always uses `MINT_AMOUNT` constant
- However, the `OperationRequested` event stores whatever `value` was passed when creating the proposal
- Old proposals were created with incorrect `value` (3000000 wei instead of 3000000 * 10^18 wei)
- UI was displaying the incorrect event value instead of the actual mint amount

**Solution:**
- For Mint operations, always use `MINT_AMOUNT` from the contract instead of the event's `value` parameter
- This is correct because the contract always mints `MINT_AMOUNT` regardless of what `value` was passed

Tasks:
- [x] Implement `getMintAmount()` method in `ContractManager` to fetch `MINT_AMOUNT` from contract
- [x] Update `formatValueForOpType()` to use `MINT_AMOUNT` for Mint operations instead of event value
- [x] Ensure `getParametersBatch()` is called before displaying Mint proposals to have `MINT_AMOUNT` available

### Step 10.2 - Dynamic Token Symbol

**Issue:** Token symbol was hardcoded as "LIB" instead of being fetched from the contract.

**Solution:**
- Fetch token symbol dynamically from contract via `symbol()` call
- Remove all hardcoded "LIB" fallbacks
- Handle `null` symbol gracefully in UI (display generic "tokens" text or omit symbol)

Tasks:
- [x] Add `symbol` to `getParametersBatch()` multicall
- [x] Update `getTokenSymbol()` to return `null` if not available (no hardcoded fallback)
- [x] Update all UI components to handle `null` symbol gracefully:
  - [x] `proposal-detail-modal.js` - Handle null symbol in value formatting
  - [x] `parameters-tab.js` - Handle null symbol in `formatLib()`
  - [x] `propose-tab.js` - Use generic "tokens" text if symbol is null
  - [x] `bridge-tab.js` - Use generic "tokens" text if symbol is null

### Step 10.3 - Cache Restoration for BigNumber Values

**Issue:** BigNumber values were being serialized/deserialized incorrectly when cached in localStorage, causing display issues.

**Solution:**
- Properly restore BigNumber instances from serialized format (`{type: 'BigNumber', hex: '...'}`)
- Defensively convert plain numbers/strings to BigNumbers when restoring from cache
- Bump cache schema version to invalidate old corrupted caches

Tasks:
- [x] Enhance `_applyCache()` in `proposals-tab.js` to properly restore BigNumber instances
- [x] Handle serialized BigNumber objects from localStorage
- [x] Handle plain numbers and strings defensively
- [x] Bump `_cacheSchemaVersion` to 3 to invalidate old caches

### Step 10.4 - Event Value Parsing

**Issue:** Ensure event values are always parsed as BigNumbers to preserve precision.

**Solution:**
- Defensively convert event values to BigNumbers in `_parseLogs()`
- Handle both BigNumber instances and serialized BigNumber objects

Tasks:
- [x] Update `_parseLogs()` to ensure values are always BigNumbers
- [x] Handle serialized BigNumber objects from event parsing
- [x] Convert plain values to BigNumbers defensively

### Step 10.5 - Proposal Creation UX Improvements

**Goal:** Improve user experience when creating proposals with better feedback and form reset.

**Current State:**
- Toast notifications for success/error are already implemented
- Input fields are not cleared after successful proposal creation

**Improvements Needed:**
- Clear all input fields (operation type, target, value, data) after successful proposal submission
- Ensure form is reset to initial state for next proposal

Tasks:
- [x] Clear operation type selector after successful proposal creation
- [x] Clear target input field after successful proposal creation
- [x] Clear value input field after successful proposal creation
- [x] Clear data input field after successful proposal creation
- [x] Reset form to initial state (hide/show fields based on operation type)

### Step 10.6 - Signing Operation Toast Notifications

**Goal:** Replace `window.alert()` with proper toast notifications for signing operations.

**Current State:**
- Signing operations use `window.alert()` for success/failure feedback
- No toast notifications for signing operations

**Improvements Needed:**
- Replace `window.alert()` with toast notifications (success/error)
...
- Refresh proposal details after successful signature submission

Tasks:
- [x] Replace `window.alert()` with toast notifications in `proposal-detail-modal.js` `onSign()` method
- [x] Add loading toast when signature transaction is submitted
- [x] Add success toast with transaction hash and explorer link when signature succeeds
- [x] Add error toast with error message when signature fails
- [x] Refresh proposal details after successful signature to show updated signature count
- [x] Re-enable sign button after transaction completes (success or failure)

### PHASE 10 DELIVERABLE

✅ Critical bugs fixed:
- Mint values now display correctly using contract's `MINT_AMOUNT`
- Token symbol is fetched dynamically from contract
- Cache restoration properly handles BigNumber serialization
- Event values are consistently parsed as BigNumbers
- Proposal creation form resets after successful submission
- Signing operations use toast notifications instead of `window.alert()`
...
### Step 9.7 - getLogs limit error handling (future-proofing)

**Problem:** RPC providers (e.g., Infura) enforce limits on `eth_getLogs` queries (typically 10,000 results). As the contract accumulates more proposals over time, a single full-range query could exceed this limit and fail.

**Solution:** Implement recursive fallback that automatically splits the block range when a limit error is detected. This ensures the app continues to work as proposal count grows without requiring manual chunking logic upfront.

**Implementation:**
- Added `getLogsWithFallback()` helper that wraps `provider.getLogs()`
- On limit error (detected via message patterns like "more than 10000 results" or RPC code -32005), recursively splits the block range in half and retries both halves
- Works transparently: normal queries (under limit) have zero overhead; only triggers when needed
- Handles nested error structures (error.error, error.data) for compatibility across providers

Tasks:
- [x] Implement `getLogsWithFallback()` with recursive range splitting
- [x] Detect common limit error patterns (Infura: "query returned more than 10000 results", RPC code -32005)
- [x] Update `_fillPendingUntil()` and `_refreshNewEventsInBackground()` to use fallback helper
- [x] Add documentation noting that error formats are based on community reports and may need extension for other providers
...
- ✅ `eth_getLogs` queries automatically handle provider limits via recursive fallback (future-proofs against large proposal counts)
...
| 10 | Bug Fixes & Improvements | Phases 4-9 |
